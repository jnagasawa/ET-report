...
$for(by-author)$\textbf{$by-author.name.literal$}$if(by-author.affiliations)$, $for(by-author.affiliations)$$by-author.affiliations.name$$if(by-author.affiliations.department)$, $by-author.affiliations.department$$endif$$if(by-author.affiliations.address)$, $for(by-author.affiliations.address)$$by-author.affiliations.address$$sep$, $endfor$$endif$$sep$; $endfor$$endif$$if(by-author.email)$, $by-author.email$$endif$$if(by-author.orcid)$ \href{https://orcid.org/$by-author.orcid$}{\orcidlink{$by-author.orcid$}}$endif$$sep$\\
